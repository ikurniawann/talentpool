          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2">
            {quickLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-white hover:bg-gray-50 transition-all group border border-gray-200">
                  <link.icon className="w-8 h-8 mb-2 text-gray-400 group-hover:text-gray-600" />
                  <span className="text-sm font-medium text-center text-gray-500 group-hover:text-gray-700">{link.label}</span>
                </div>
              </Link>
            ))}
          </div>
