...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const required = ["kode", "nama", "satuan_pembelian"];
      ...
      const { data: existing } = await supabase.from("raw_materials").select("id").eq("kode", rowData.kode).single();
      if (existing) {
        errors.push({ row: rowNumber, message: `Kode bahan baku ${rowData.kode} sudah ada` });
        skipped.push({ row: rowNumber, data: rowData });
        continue;
      }

      const rawData = {
        kode: rowData.kode, nama: rowData.nama, nama_lain: rowData.nama_lain || null,
        kategori: rowData.kategori || null, satuan_pembelian: rowData.satuan_pembelian,
        satuan_penggunaan: rowData.satuan_penggunaan || rowData.satuan_pembelian,
        qty_per_unit: parseFloat(rowData.qty_per_unit) || 1,
        harga_rata_rata: parseFloat(rowData.harga_rata_rata) || 0,
        stok_minimum: parseInt(rowData.stok_minimum) || 0,
        stok_maksimum: parseInt(rowData.stok_maksimum) || 0,
        tanggal_mulai_produksi: rowData.tanggal_mulai_produksi || null,
        masa_simpan: parseInt(rowData.masa_simpan) || 0,
        supplier_utama: rowData.supplier_utama || null,
        deskripsi: rowData.deskripsi || null,
        is_active: rowData.status?.toLowerCase() === "active",
      };

      const { error } = await supabase.from("raw_materials").insert(rawData);
