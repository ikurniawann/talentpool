...
export default function LogbookListPage() {
  ...
  async function loadData() {
    ...
    setMessage("");
    try {
      const params = new URLSearchParams({ resource: "entries" });
      if (effectiveDepartment) params.set("department_id", effectiveDepartment);
      if (selectedDate) params.set("date", selectedDate);

      const [meRes, departmentRes, entryRes] = await Promise.all([
        fetchJson("/api/hris/logbook?resource=me"),
        fetchJson("/api/hris/logbook?resource=departments"),
        fetchJson(`/api/hris/logbook?${params.toString()}`),
      ]);
      ...
      setMessage(error instanceof Error ? error.message : "Gagal memuat logbook list");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggleItem(item: EntryItem, checked: boolean) {
    try {
      await fetchJson("/api/hris/logbook", {
        method: "PATCH",
        body: JSON.stringify({ action: "update-item", item_id: item.id, is_checked: checked }),
      });
      await loadData();
    } catch (error) {
  ...
  async function saveNote() {
    ...
    try {
      await fetchJson("/api/hris/logbook", {
        method: "PATCH",
        body: JSON.stringify({ action: "update-item", item_id: noteItem.id, notes: noteDraft }),
      });
  ...
  async function submitEntry(entryId: string) {
    ...
    try {
      await fetchJson("/api/hris/logbook", {
        method: "PATCH",
        body: JSON.stringify({ action: "submit-entry", entry_id: entryId }),
      });
