...
export default function NewPerformanceReviewPage() {
  ...
  const [selectedTemplateDetail, setSelectedTemplateDetail] = useState<KpiTemplate | null>(null);
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      const res = await fetch("/api/hris/employees?limit=100&status=active");
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      const res = await fetch("/api/hris/kpi-templates?limit=100");
      const json = await res.json();
      if (res.ok) setTemplates((json.data || []).filter((template: KpiTemplate) => template.status !== "archived"));
    } catch {
      // silent
    }
  }, []);
  ...
  useEffect(() => {
    if (selectedTemplate) {
      const loadTemplateDetail = async () => {
        const res = await fetch(`/api/hris/kpi-templates/${selectedTemplate}`);
        const json = await res.json();
        if (!res.ok) return;
        const template = json.data as KpiTemplate;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(json.error || "Gagal menyimpan review", "error");
        setLoading(false);
        return;
      }
