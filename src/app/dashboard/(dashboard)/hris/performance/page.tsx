...
export default function PerformancePage() {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      ...
      if (periodFilter !== "all") params.set("period_label", periodFilter);
      if (statusFilter !== "all") params.set("status", statusFilter);
      params.set("limit", "50");

      const res = await fetch(`/api/hris/performance/reviews?${params.toString()}`);
      const json = await res.json();

      if (res.ok) {
        setReviews(json.data || []);
        const uniquePeriods = [...new Set((json.data || []).map((r: PerformanceReview) => r.period_label))] as string[];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={() => router.push("/dashboard/hris/performance/new")} className="gap-2">
