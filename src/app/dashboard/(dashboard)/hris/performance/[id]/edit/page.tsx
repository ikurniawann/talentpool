...
export default function EditPerformanceReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!res.ok) {
          toast({ title: "Error", description: json.error || "Gagal memuat data", variant: "destructive" });
          setLoading(false);
          return;
        }
        ...
        // Fetch KPIs
        const kpiRes = await fetch(`/api/hris/performance/employee-kpis?employee_id=${review.employee_id}&period_label=${review.period_label}`);
        const kpiJson = await kpiRes.json();
        if (kpiRes.ok && kpiJson.data) {
          setKpis(kpiJson.data.map((k: any) => ({
            id: k.id,
        ...
        // Fetch behavioral
        const behRes = await fetch(`/api/hris/performance/behavioral?employee_id=${review.employee_id}&review_period=${review.period_label}`);
        const behJson = await behRes.json();
        if (behRes.ok && behJson.data) {
          const b = behJson.data;
          setBehaviorals([
        ...
        const devRes = await fetch(`/api/hris/performance/development-plans?employee_id=${review.employee_id}&review_period=${review.period_label}`);
        const devJson = await devRes.json();
        if (devRes.ok && devJson.data) {
          setDevelopments(devJson.data.map((d: any) => ({
        ...
        const empRes = await fetch("/api/hris/employees?limit=100&status=active");
        const empJson = await empRes.json();
        if (empRes.ok) setEmployees(empJson.data || []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/hris/performance/reviews/${reviewId}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        const json = await res.json();
        toast({ title: "Error", description: json.error || "Gagal update review", variant: "destructive" });
        setSaving(false);
        return;
      }
      ...
      for (const kpi of kpis) {
        const { score } = calculateKpiScore(kpi);
        if (kpi.id) {
          await fetch("/api/hris/performance/employee-kpis", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: kpi.id,
