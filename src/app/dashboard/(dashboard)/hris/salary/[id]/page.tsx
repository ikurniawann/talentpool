...
export default function EditSalaryPage({ params }: PageProps) {
  ...
  async function fetchSalary() {
    try {
      const res = await fetch(`/api/hris/employee-salary/${id}`);
      const json = await res.json();
      
      if (!res.ok) {
        showToast(json.error || "Gagal memuat data salary", "error");
        return;
      }
      ...
      if (json.data) {
        setSalary(json.data);
        setFormData({
          base_salary: String(json.data.base_salary || ""),
          fixed_allowance: String(json.data.fixed_allowance || 0),
          variable_allowance: String(json.data.variable_allowance || 0),
          transport_allowance: String(json.data.transport_allowance || 0),
          meal_allowance: String(json.data.meal_allowance || 0),
          housing_allowance: String(json.data.housing_allowance || 0),
          loan_deduction: String(json.data.loan_deduction || 0),
          other_deduction: String(json.data.other_deduction || 0),
          ptkp_status: json.data.ptkp_status || "TK/0",
          is_taxable: json.data.is_taxable ?? true,
          bpjs_tk_enrolled: json.data.bpjs_tk_enrolled ?? true,
          bpjs_kes_enrolled: json.data.bpjs_kes_enrolled ?? true,
          tapera_enrolled: json.data.tapera_enrolled ?? true,
