...
export default function PRApprovalPage() {
  ...
  const [prs, setPrs] = useState<ApprovalPR[]>([]);
  ...
  async function loadPendingPRs() {
    setLoading(true);
    try {
      const response = await fetch("/api/purchasing/pr?status=pending_head&limit=50");
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Gagal memuat approval PR");
      setPrs(payload.data || []);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Gagal memuat approval PR");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function approvePR() {
    ...
    setProcessingId(confirmingPR.id);
    try {
      const response = await fetch(`/api/purchasing/pr/${confirmingPR.id}/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "approve" }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Gagal approve PR");
    ...
    <div className="space-y-6">
      <BreadcrumbNav
        items={[
          ...
          { label: "Approval", href: "/dashboard/purchasing/approval" },
          { label: "Approval PR" },
        ]}
      />
