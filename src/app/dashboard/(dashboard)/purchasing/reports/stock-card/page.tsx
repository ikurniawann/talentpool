...
type MovementType = "all" | "in" | "out" | "adjustment" | "transfer" | "return";
...
export default function StockCardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (selectedMaterial !== "all") params.set("material_id", selectedMaterial);
      if (appliedSearch) params.set("search", appliedSearch);
      if (movementType !== "all") params.set("tipe", movementType);
      if (dateFrom) params.set("date_from", dateFrom);
      if (dateTo) params.set("date_to", dateTo);
      params.set("limit", "500");

      const response = await fetch(`/api/purchasing/reports/stock-card?${params.toString()}`);
      ...
      toast.error(error instanceof Error ? error.message : "Gagal memuat stock card");
    } finally {
      setLoading(false);
    }
  }, [appliedSearch, dateFrom, dateTo, movementType, selectedMaterial]);

  useEffect(() => {
    if (urlFilterReady) loadData();
  }, [loadData, urlFilterReady]);
