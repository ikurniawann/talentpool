  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] gap-4">
      ...
      <Dialog open={showPaymentModal} onOpenChange={(open) => !open && setShowPaymentModal(false)}>
        <DialogContent className="!w-[800px] !max-w-[800px] max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-3 py-2 overflow-y-auto flex-1">
            <div className="text-center py-3 bg-gray-900 rounded-lg">
              <div className="text-xs text-gray-400 mb-1">Total Pembayaran</div>
              <div className="text-lg font-bold text-white">{formatBoth(total)}</div>
            </div>
            ...
            <div className="flex border-b border-gray-200">
              {[{ id: 'member', label: 'Kartu Member', icon: CreditCard }, { id: 'cash', label: 'Tunai', icon: Banknote }, { id: 'qris', label: 'QRIS', icon: Wallet }, { id: 'split', label: 'Split', icon: Coins }].map(method => (
                <button key={method.id} onClick={() => setPaymentMethod(method.id as PaymentMethod)} className={`flex-1 py-2 text-xs font-semibold border-b-2 ${paymentMethod === method.id ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-500'}`}>
                  <method.icon className="w-4 h-4 mx-auto mb-0.5" />
                  {method.label}
                </button>
              ))}
            </div>
            ...
            {paymentMethod === 'member' && (
              <div className="text-center py-4">
                {selectedCustomer ? (
                  ...
                  <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200 text-xs text-yellow-700">Pilih pelanggan terlebih dahulu untuk menggunakan Kartu Member</div>
                )}
              </div>
            )}
            ...
            {paymentMethod === 'qris' && (
              <div className="text-center py-4">
                <div className="w-40 h-40 bg-gray-100 rounded-lg mx-auto mb-3 flex items-center justify-center border-2 border-dashed border-gray-300"><span className="text-gray-400 text-xs">QR Code</span></div>
                <p className="text-xs text-gray-500">Scan QRIS untuk membayar</p>
              </div>
            )}
            ...
            {paymentMethod === 'split' && (
              <div className="space-y-2">
                <div className="p-2 bg-blue-50 rounded-lg border border-blue-200 flex justify-between"><span className="text-xs text-gray-700 font-medium">Sisa Bayar</span><span className="text-xs font-bold text-blue-600">{formatCurrency(remainingSplit)}</span></div>
                <div className="w-full bg-gray-200 rounded-full h-1.5"><div className="bg-pink-600 h-1.5 rounded-full transition-all" style={{ width: `${Math.min(100, (getTotalSplitPaid() / total) * 100)}%` }} /></div>
                {splitPayments.map((split) => (
                  <div key={split.id} className="flex gap-2 items-center">
                    <select value={split.method} onChange={(e) => updateSplitPayment(split.id, 'method', e.target.value)} className="px-2 py-1.5 border border-gray-300 rounded-lg text-xs">
                    ...
                    <input type="number" value={split.amount} onChange={(e) => updateSplitPayment(split.id, 'amount', e.target.value)} className="flex-1 px-2 py-1.5 border border-gray-300 rounded-lg text-xs" placeholder="Jumlah" />
                    <button onClick={() => removeSplitPayment(split.id)} className="p-1.5 text-gray-400 hover:text-red-600"><MinusCircle className="w-4 h-4" /></button>
                  </div>
                ))}
                <button onClick={addSplitPayment} disabled={remainingSplit <= 0} className="w-full py-1.5 border-2 border-dashed border-gray-300 rounded-lg text-xs text-gray-500 hover:border-pink-400 hover:text-pink-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"><Plus className="w-3 h-3" />Tambah</button>
              </div>
            )}

            <div className="flex items-center justify-between pt-3 border-t border-gray-200">
              <span className="text-xs font-semibold text-gray-900">Tip</span>
              <button onClick={() => setShowTipInput(!showTipInput)} className={`px-2 py-1 rounded-lg text-xs font-medium ${showTipInput ? 'bg-pink-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{showTipInput ? 'Sembunyikan' : 'Tambah'}</button>
            </div>
            {showTipInput && <input type="number" value={tipAmount} onChange={(e) => setTipAmount(e.target.value)} placeholder="Jumlah tip" className="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500" />}

            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-gray-900">Struk Digital</span>
              <button onClick={() => setDigitalReceipt(!digitalReceipt)} className={`px-2 py-1 rounded-lg text-xs font-medium ${digitalReceipt ? 'bg-pink-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{digitalReceipt ? 'Aktif' : 'Nonaktif'}</button>
            </div>
          </div>
          <div className="flex-shrink-0 flex gap-3 pt-3 border-t">
            <button onClick={() => setShowPaymentModal(false)} className="flex-1 px-3 py-2 text-gray-700 bg-gray-100 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">Batal</button>
            <button onClick={completeOrder} disabled={!isPaymentValid()} className="flex-1 px-3 py-2 text-white bg-pink-600 rounded-lg text-sm font-medium hover:bg-pink-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors flex items-center gap-2"><Check className="w-4 h-4" />Bayar</button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
