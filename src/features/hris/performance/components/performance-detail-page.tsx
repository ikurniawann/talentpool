...
import { usePerformanceReview } from "../queries";
import { useDeletePerformanceReview } from "../mutations";
...
export function PerformanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const { toasts, showToast, removeToast } = useToast();
  const { id } = use(params);

  const reviewQuery = usePerformanceReview(id);
  const review = (reviewQuery.data as PerformanceReviewDetail | undefined) ?? null;
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="default" onClick={() => router.push(`/dashboard/hris/performance/edit/${review.id}`)}>
