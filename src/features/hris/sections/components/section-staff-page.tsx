...
import { useSectionStaff, useSectionsList, useStaffSections, useSectionBrands } from "../queries";
import {
  useCreateSection,
  useAssignStaffSection,
  useRemoveStaffSection,
  useDeleteSection,
} from "../mutations";
import type { SectionItem, SectionStaffMember } from "../types";

export function SectionStaffPage() {
  ...
  // Dialogs
  const [sectionDialog, setSectionDialog] = useState<SectionStaffMember | null>(null);
  ...
  const staffQuery = useSectionStaff(brandFilter);
  const sectionsQuery = useSectionsList(brandFilter);
  const staffSectionsQuery = useStaffSections();
  const brandsQuery = useSectionBrands();

  const staff = staffQuery.data ?? [];
  const sections = sectionsQuery.data ?? [];
  const staffSections = staffSectionsQuery.data ?? [];
  const brands = brandsQuery.data ?? [];
  const loading =
    staffQuery.isLoading ||
    sectionsQuery.isLoading ||
    staffSectionsQuery.isLoading ||
    brandsQuery.isLoading;

  const createSectionMutation = useCreateSection();
  const assignMutation = useAssignStaffSection();
  const removeMutation = useRemoveStaffSection();
  const deleteSectionMutation = useDeleteSection();
  const saving = createSectionMutation.isPending || assignMutation.isPending;

  function reload() {
    staffQuery.refetch();
    sectionsQuery.refetch();
    staffSectionsQuery.refetch();
    brandsQuery.refetch();
  }
  ...
  async function handleCreateSection() {
    ...
    try {
      ...
      showToast("Gagal membuat section: " + (err instanceof Error ? err.message : ""), "error");
    }
  }

  async function handleAssignSection(st: SectionStaffMember | null) {
    if (!st || !assignSection) return;
    try {
      await assignMutation.mutateAsync({ staff_id: st.id, section_id: assignSection });
      showToast("Staff berhasil ditempatkan di section");
      setSectionDialog(null);
      setAssignSection("");
    } catch (err) {
      showToast("Gagal menempatkan staff: " + (err instanceof Error ? err.message : ""), "error");
    }
  }

  async function handleRemoveSection(staffId: string, sectionId: string) {
    try {
      await removeMutation.mutateAsync({ staffId, sectionId });
      showToast("Staff berhasil dikeluarkan dari section");
    } catch (err) {
      showToast("Gagal menghapus staff dari section: " + (err instanceof Error ? err.message : ""), "error");
    }
  }
  ...
  async function handleDeleteSection(sec: SectionItem) {
    ...
    try {
      ...
      showToast("Gagal hapus section: " + (err instanceof Error ? err.message : ""), "error");
    }
  }
