...
BEGIN
  ...
  SELECT LPAD(COALESCE(MAX(
    CAST(SUBSTRING(nip FROM 'EMP-' || year || '-(\d+)$' AS INTEGER)
  ), 0) + 1)::TEXT, 5, '0')
  INTO sequence
  FROM employees
  WHERE nip LIKE 'EMP-' || year || '-%';
  
  -- Build NIP
  new_nip := 'EMP-' || year || '-' || sequence;
...
BEGIN
  ...
  -- Get position and brand from candidate
  v_position_id := v_candidate.position_id;
  v_brand_id := v_candidate.brand_id;
...
BEGIN
  ...
  RETURN v_role IN ('hrd', 'hiring_manager');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Manager: Read employees in their department + their own
CREATE POLICY "Manager read employees in department"
  ON employees FOR SELECT
  USING (
    is_manager() = true AND
    (department_id IN (
      SELECT id FROM departments WHERE brand_id IN (
...
COMMENT ON COLUMN employees.promoted_to_employee_id IS 'Link ke employee record jika kandidat dipromosikan';
